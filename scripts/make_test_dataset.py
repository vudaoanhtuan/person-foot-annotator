...
Creates:
  ...
  <output_dir>/records.sqlite     records table; most rows unannotated,
...
def main() -> None:
    ...
    conn = sqlite3.connect(out / "records.sqlite")
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS records (
          ...
          foot_x REAL,
          foot_y REAL,
          bb_in_context_x_min REAL,
          bb_in_context_y_min REAL,
          bb_in_context_x_max REAL,
          bb_in_context_y_max REAL
        )
        """
    )
    ...
    for i in range(n):
        ...
        conn.execute(
            ...
            (name, foot[0], foot[1], x_min, y_min, x_max, y_max),
        )
